...
const createProducts = async (req, res) => {
    try {
        let { name, description, price, category, sizes, stock, isFeatured, collab, sizeInventory } = req.body;
        ...
        // Parse sizeInventory if sent as JSON string
        let parsedSizeInventory = {};
        if (sizeInventory) {
            try {
                parsedSizeInventory = typeof sizeInventory === 'string'
                    ? JSON.parse(sizeInventory)
                    : sizeInventory;
            } catch { parsedSizeInventory = {}; }
        }

        // Calculate total stock from sizeInventory if provided
        const totalStock = Object.keys(parsedSizeInventory).length > 0
            ? Object.values(parsedSizeInventory).reduce((sum, qty) => sum + (Number(qty) || 0), 0)
            : (stock || 0);
