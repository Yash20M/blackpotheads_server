...
const getallproducts = async (req, res) => {
  try {
    ...
    const formattedProducts = products.map(product => ({
      ...
      sizes: product.sizes,
      sizeInventory: product.sizeInventory ? Object.fromEntries(product.sizeInventory) : {},
      images: product.images,
      stock: product.stock,
      isFeatured: product.isFeatured,
      collab: product.collab || null,
...
const updateproduct = async (req, res) => {
  try {
    ...
    const { name, price, description, category, sizes, stock, isFeatured, collab, sizeInventory } = req.body;
    const updateData = {};
    ...
    // Handle sizeInventory
    if (sizeInventory !== undefined) {
      try {
        const parsed = typeof sizeInventory === 'string' ? JSON.parse(sizeInventory) : sizeInventory;
        updateData.sizeInventory = parsed;
        // Auto-calculate total stock from sizeInventory
        updateData.stock = Object.values(parsed).reduce((sum, qty) => sum + (Number(qty) || 0), 0);
